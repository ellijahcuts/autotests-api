...
def assert_get_user_response(get_user_response: GetUserResponseSchema, create_user_response: CreateUserResponseSchema):
    """
    Проверяет, что данные пользователя при создании и при GET-запросе совпадают.

    :param get_user_response: Ответ от GET /api/v1/users/{user_id}
    :param create_user_response: Ответ от POST /api/v1/users (создание пользователя)
    """
